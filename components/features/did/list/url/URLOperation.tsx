...
export function URLOperation({
    ...
    return (
        <div className="bg-[#cbd5e1] dark:bg-slate-900 border border-slate-300 dark:border-slate-800 rounded shadow-sm overflow-hidden">
            <Tabs value={operation} onValueChange={(v) => onOperationChange(v as OperationType)} className="w-full">
                ...
                <TabsContent value="CREATE" className="m-0">
                    <div className="p-6 space-y-4">
                        <Label className="text-[11px] font-bold text-slate-500 uppercase tracking-widest">Content</Label>
                        <p className="text-[10px] text-slate-400 font-medium">Base64-encoded data associated with the DID URL</p>
                        <Textarea
                            value={content}
                ...
                <TabsContent value="UPDATE" className="m-0">
                    <div className="grid grid-cols-1 lg:grid-cols-2 bg-[#cbd5e1] dark:bg-slate-900">
                        <div className="p-6 space-y-4 border-r border-slate-300 dark:border-slate-700">
                            <Label className="text-[11px] font-bold text-slate-500 uppercase tracking-widest">Content</Label>
                            <p className="text-[10px] text-slate-400 font-medium leading-relaxed">Base64-encoded data associated with the DID URL</p>
                            <Textarea
                                value={content}
                                onChange={(e) => onContentChange(e.target.value)}
                                className="h-[300px] font-mono text-xs bg-white dark:bg-slate-950 resize-none shadow-inner"
                        ...
                        <div className="p-6 space-y-4">
                            <div className="flex items-center justify-between">
                                <div className="space-y-1">
                                    <Label className="text-[11px] font-bold text-slate-500 uppercase tracking-widest">Content Operations</Label>
                                    <p className="text-[10px] text-slate-400 font-medium">Update operations</p>
                                </div>
                                <Badge variant="outline" className="text-[9px] font-bold opacity-50">JSON</Badge>
                            </div>
                            <div className="border rounded-xl overflow-hidden bg-white dark:bg-slate-950 min-h-[450px]">
                                <JSONEditor value={contentOperations} onChange={onContentOperationsChange} height="300px" />
                            </div>
                        </div>
                    </div>
                </TabsContent>
